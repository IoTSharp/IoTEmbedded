...
static int basic_uart_open(struct mb_interpreter_t *s, void **l);
...
static int basic_uart_pop_port(struct mb_interpreter_t *s, void **l, basic_uart_port_t *port);
static int basic_uart_write_after_port(struct mb_interpreter_t *s, void **l, basic_uart_port_t port);
static int basic_uart_write_bytes_after_port(struct mb_interpreter_t *s, void **l, basic_uart_port_t port);
static int basic_uart_read_after_port(struct mb_interpreter_t *s, void **l, basic_uart_port_t port);
static int basic_uart_read_bytes_after_port(struct mb_interpreter_t *s, void **l, basic_uart_port_t port);
static int basic_uart_baud_after_port(struct mb_interpreter_t *s, void **l, basic_uart_port_t port);
static int basic_uart_set_baud_after_port(struct mb_interpreter_t *s, void **l, basic_uart_port_t port);
static int basic_uart_flush_after_port(struct mb_interpreter_t *s, void **l, basic_uart_port_t port);
static int basic_uart_push_port(struct mb_interpreter_t *s, void **l, uint8_t port_id,
                                basic_uart_interface_t interface);
static bool basic_uart_port_id_from_int(int_t value, uint8_t *port_id);
static bool basic_uart_interface_from_name(const char *name, basic_uart_interface_t *interface);
static bool basic_uart_name_equals(const char *left, const char *right);
static const basic_uart_port_t *basic_uart_port_descriptor(uint8_t port_id, basic_uart_interface_t interface);
static bool basic_uart_port_from_usertype(void *value, basic_uart_port_t *port);
static bool basic_uart_is_rs485_with_direction(basic_uart_port_t port);
static void basic_uart_begin_write(basic_uart_port_t port);
static HAL_StatusTypeDef basic_uart_end_write(basic_uart_port_t port, HAL_StatusTypeDef status);
static HAL_StatusTypeDef basic_uart_write_chunk(basic_uart_port_t port, const uint8_t *data, uint16_t length,
                                                uint32_t timeout_ms);
static uint32_t basic_uart_remaining_timeout(uint32_t start_ms, uint32_t timeout_ms);
static int basic_uart_write_array(struct mb_interpreter_t *s, void **l, basic_uart_port_t port, void *array,
...
static bool basic_uart_array_capacity(struct mb_interpreter_t *s, void **l, mb_value_t array_value,
                                      uint32_t *capacity);
static HAL_StatusTypeDef basic_uart_read_one(basic_uart_port_t port, uint8_t *byte, uint32_t timeout_ms);
static bool basic_uart_next_read_slice(uint32_t start_ms, uint32_t timeout_ms, uint32_t *slice_ms);
static int basic_uart_read_to_array(struct mb_interpreter_t *s, void **l, basic_uart_port_t port, void *array,
...
static const basic_uart_port_t basic_uart_port_descriptors[] = {
  {BASIC_UART_INTERFACE_RS485, BSP_UART1_HANDLE, 1U},
  {BASIC_UART_INTERFACE_UART, BSP_UART2_HANDLE, 2U},
  {BASIC_UART_INTERFACE_UART, BSP_UART4_HANDLE, 4U},
  {BASIC_UART_INTERFACE_RS232, BSP_UART5_HANDLE, 5U},
};
...
ErrorStatus basic_uart_register(struct mb_interpreter_t *interpreter) {
  ...
  result |= mb_register_func(interpreter, "UART_OPEN", basic_uart_open);
...
static int basic_uart_open(struct mb_interpreter_t *s, void **l) {
  ...
  uint8_t port_id = 0U;
  basic_uart_interface_t interface = BASIC_UART_INTERFACE_NONE;
  mb_check(mb_attempt_open_bracket(s, l));
  mb_check(mb_pop_int(s, l, &port_arg));
  mb_check(mb_pop_string(s, l, &interface_name));
  mb_check(mb_attempt_close_bracket(s, l));
  if (!basic_uart_port_id_from_int(port_arg, &port_id) ||
      !basic_uart_interface_from_name(interface_name, &interface)) {
    return MB_FUNC_ERR;
  }
  return basic_uart_push_port(s, l, port_id, interface);
}
...
static int basic_uart_write(struct mb_interpreter_t *s, void **l) {
  ...
  mb_check(mb_attempt_open_bracket(s, l));
  mb_check(basic_uart_pop_port(s, l, &port));
  return basic_uart_write_after_port(s, l, port);
}
...
static int basic_uart_write_bytes(struct mb_interpreter_t *s, void **l) {
  ...
  mb_check(mb_attempt_open_bracket(s, l));
  mb_check(basic_uart_pop_port(s, l, &port));
  return basic_uart_write_bytes_after_port(s, l, port);
}
...
static int basic_uart_read(struct mb_interpreter_t *s, void **l) {
  ...
  mb_check(mb_attempt_open_bracket(s, l));
  mb_check(basic_uart_pop_port(s, l, &port));
  return basic_uart_read_after_port(s, l, port);
}
...
static int basic_uart_read_bytes(struct mb_interpreter_t *s, void **l) {
  ...
  mb_check(mb_attempt_open_bracket(s, l));
  mb_check(basic_uart_pop_port(s, l, &port));
  return basic_uart_read_bytes_after_port(s, l, port);
}
...
static int basic_uart_baud(struct mb_interpreter_t *s, void **l) {
  ...
  mb_check(mb_attempt_open_bracket(s, l));
  mb_check(basic_uart_pop_port(s, l, &port));
  return basic_uart_baud_after_port(s, l, port);
}
...
static int basic_uart_set_baud(struct mb_interpreter_t *s, void **l) {
  ...
  mb_check(mb_attempt_open_bracket(s, l));
  mb_check(basic_uart_pop_port(s, l, &port));
  return basic_uart_set_baud_after_port(s, l, port);
}
...
static int basic_uart_flush_func(struct mb_interpreter_t *s, void **l) {
  ...
  mb_check(mb_attempt_open_bracket(s, l));
  mb_check(basic_uart_pop_port(s, l, &port));
  return basic_uart_flush_after_port(s, l, port);
}
...
static int basic_uart_pop_port(struct mb_interpreter_t *s, void **l, basic_uart_port_t *port) {
  ...
  bool ok = basic_uart_port_from_value(port_value, port);
...
static int basic_uart_write_after_port(struct mb_interpreter_t *s, void **l, basic_uart_port_t port) {
  ...
  size_t written = basic_uart_write_data(port, (const uint8_t *)text, strlen(text), timeout_ms);
  basic_uart_feed_heartbeat();
...
static int basic_uart_write_bytes_after_port(struct mb_interpreter_t *s, void **l, basic_uart_port_t port) {
  ...
  if (array_value.type == MB_DT_ARRAY && array_value.value.array != NULL && length_arg > 0) {
    ...
    if (basic_uart_array_capacity(s, l, array_value, &capacity)) {
      ...
      result = basic_uart_write_array(s, l, port, array_value.value.array, length,
...
static int basic_uart_read_after_port(struct mb_interpreter_t *s, void **l, basic_uart_port_t port) {
  ...
  uint16_t read_count = basic_uart_read_data(port, buffer, length, basic_uart_timeout_from_int(timeout_arg));
  buffer[read_count] = '\0';
  basic_uart_feed_heartbeat();
...
static int basic_uart_read_bytes_after_port(struct mb_interpreter_t *s, void **l, basic_uart_port_t port) {
  ...
  if (array_value.type == MB_DT_ARRAY && array_value.value.array != NULL && length_arg > 0) {
    ...
    if (basic_uart_array_capacity(s, l, array_value, &capacity)) {
      ...
      result = basic_uart_read_to_array(s, l, port, array_value.value.array, length,
...
static int basic_uart_baud_after_port(struct mb_interpreter_t *s, void **l, basic_uart_port_t port) {
  mb_check(mb_attempt_close_bracket(s, l));
  return mb_push_int(s, l, basic_uart_u32_to_int(basic_uart_get_baud_rate(port)));
}
...
static int basic_uart_set_baud_after_port(struct mb_interpreter_t *s, void **l, basic_uart_port_t port) {
  ...
  HAL_StatusTypeDef status = basic_uart_configure_baud_rate(port, (uint32_t)baud_arg);
  basic_uart_feed_heartbeat();
...
bool basic_uart_port_from_value(mb_value_t value, basic_uart_port_t *port) {
  ...
  return basic_uart_port_from_usertype(value.value.usertype, port);
}

bool basic_uart_port_is_modbus_capable(basic_uart_port_t port) {
  return port.handle != NULL &&
         (port.interface == BASIC_UART_INTERFACE_RS485 || port.interface == BASIC_UART_INTERFACE_RS232);
}

static int basic_uart_push_port(struct mb_interpreter_t *s, void **l, uint8_t port_id,
                                basic_uart_interface_t interface) {
  const basic_uart_port_t *port = basic_uart_port_descriptor(port_id, interface);
...
static bool basic_uart_interface_from_name(const char *name, basic_uart_interface_t *interface) {
  ...
  if (basic_uart_name_equals(name, "UART") || basic_uart_name_equals(name, "TTL")) {
    *interface = BASIC_UART_INTERFACE_UART;
    return true;
  }
  if (basic_uart_name_equals(name, "RS485")) {
    *interface = BASIC_UART_INTERFACE_RS485;
    return true;
  }
  if (basic_uart_name_equals(name, "RS232")) {
    *interface = BASIC_UART_INTERFACE_RS232;
    return true;
  }
...
static const basic_uart_port_t *basic_uart_port_descriptor(uint8_t port_id, basic_uart_interface_t interface) {
  for (size_t i = 0U; i < (sizeof(basic_uart_port_descriptors) / sizeof(basic_uart_port_descriptors[0])); i++) {
    if (basic_uart_port_descriptors[i].port_id == port_id &&
        basic_uart_port_descriptors[i].interface == interface) {
      return &basic_uart_port_descriptors[i];
    }
  }
  return NULL;
}
...
static bool basic_uart_port_from_usertype(void *value, basic_uart_port_t *port) {
  ...
  for (size_t i = 0U; i < (sizeof(basic_uart_port_descriptors) / sizeof(basic_uart_port_descriptors[0])); i++) {
    if (value == (void *)&basic_uart_port_descriptors[i]) {
      *port = basic_uart_port_descriptors[i];
      return port->handle != NULL;
    }
  }
  return false;
...
static bool basic_uart_is_rs485_with_direction(basic_uart_port_t port) {
  return port.interface == BASIC_UART_INTERFACE_RS485 && port.handle == BSP_RS485_UART_HANDLE;
}

basic_uart_port_t basic_uart_rs485_port(void) {
  const basic_uart_port_t *port = basic_uart_port_descriptor(1U, BASIC_UART_INTERFACE_RS485);
  if (port != NULL) {
    return *port;
  }
  basic_uart_port_t empty = {BASIC_UART_INTERFACE_NONE, NULL, 0U};
  return empty;
}

uint32_t basic_uart_get_baud_rate(basic_uart_port_t port) {
  if (basic_uart_is_rs485_with_direction(port)) {
...
size_t basic_uart_write_data(basic_uart_port_t port, const uint8_t *data, size_t length, uint32_t timeout_ms) {
  ...
  while (written < length) {
    ...
    basic_uart_feed_heartbeat();
    status = basic_uart_write_chunk(port, &data[written], chunk_length, chunk_timeout);
    if (status != HAL_OK) {
      break;
    }
    written += chunk_length;
  }
  (void)basic_uart_end_write(port, status);
  return written;
}
...
static int basic_uart_write_array(struct mb_interpreter_t *s, void **l, basic_uart_port_t port, void *array,
  ...
  while (offset < length) {
    ...
    basic_uart_feed_heartbeat();
    status = basic_uart_write_chunk(port, chunk, chunk_length, chunk_timeout);
    if (status != HAL_OK) {
      break;
    }
    offset += chunk_length;
...
uint16_t basic_uart_read_data(basic_uart_port_t port, uint8_t *buffer, uint16_t length, uint32_t timeout_ms) {
  ...
  while (read_count < length) {
    ...
    basic_uart_feed_heartbeat();
    HAL_StatusTypeDef status = basic_uart_read_one(port, &buffer[read_count], slice_ms);
    if (status == HAL_OK) {
      read_count++;
      continue;
    }
...
static int basic_uart_read_to_array(struct mb_interpreter_t *s, void **l, basic_uart_port_t port, void *array,
  ...
  *read_count = 0U;
  ...
  while (*read_count < length) {
    ...
    basic_uart_feed_heartbeat();
    HAL_StatusTypeDef status = basic_uart_read_one(port, &byte, slice_ms);
    if (status == HAL_OK) {
